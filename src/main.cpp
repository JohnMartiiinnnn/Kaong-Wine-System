...
bool ljRight = false, ljLeft = false, ljUp = false, ljDown = false, ljSelect = false;
bool lastLjRight = false, lastLjLeft = false, lastLjUp = false, lastLjDown = false, lastLjSelect = false;
...
int menuSelection = 0; // 0: New Brew, 1: Continue, 2: System Check, 3: Sensor Values
...
void logDataToSD()
{
    ...
    if (dataFile)
    {
        ...
        dataFile.print(',');
        dataFile.print(liquid1Status ? sharedLiquidSensors.getTempCByIndex(0) : 0.0);
        dataFile.print(',');
        dataFile.print(liquid2Status ? sharedLiquidSensors.getTempCByIndex(1) : 0.0);
        dataFile.print(',');
        dataFile.print(incomingData.room2Temp);
        dataFile.print(',');
        dataFile.print(incomingData.ds18Status == 1 ? incomingData.room2LiquidTemp : 0.0);
        dataFile.print(',');
...
void drawNewBrewWizard()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        { // Proceed Button
            if (canProceed)
                bgColor = (wizardSelection == 0) ? 0x03E0 : 0x0400; // Green / Dark Green
            else
...
void drawDashboardLayout()
{
    ...
    if (!moduleViewActive)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                tft.drawRect(3, y - 2, 314, 54, TFT_WHITE); // erase old selection border
            }
...
void drawSensorMonitorPage()
{
    ...
    sprintf(b, "R:%d L:%d U:%d D:%d S:%d W:%ld", ljRight, ljLeft, ljUp, ljDown, ljSelect, sRaw);
...
void setup()
{
    ...
    {
        hx711Status = true;
        scale.set_scale(calibrationFactor); // Use the variable
        scale.set_offset(loadCellOffset);   // Constant tare
    }
...
{
    server.handleClient();
    static uint32_t ld = 0, ll = 0, ls = 0;
    ...
    if (cRight && !ljRight && currentAppState == CALIBRATION_MODE && calSelection == 1)
    {
    ...
    if (hx711Status && scale.is_ready())
    {
        ...
        currentWeight = (currentWeight * 0.9) + (rawW * 0.1); // Exponential moving average
        if (currentWeight > -0.15 && currentWeight < 0.15)
        {
            currentWeight = 0.0; // Deadband snap to zero
        }
    }
    ...
    if (millis() - ld > 2000)
    {
        ...
        if (currentAppState == DASHBOARD_ACTIVE && moduleViewActive)
        {
            ...
            if (dashSelection == 0)
            {
                ...
                String pl = liquid1Status ? String(sharedLiquidSensors.getTempCByIndex(0), 1) : "--";
                ...
                tft.drawCentreString(String(currentHeatingPercent) + "%", 240, y + 160, 4);
            ...
            else if (dashSelection == 1)
            {
                ...
                tft.setTextPadding(140);
                String fa = (incomingData.sensor2Status > 0) ? String(incomingData.room2Temp, 1) : "--";
                String fl = (incomingData.ds18Status == 1) ? String(incomingData.room2LiquidTemp, 1) : "--";
            ...
            else if (dashSelection == 2)
            {
                ...
                String pt = liquid2Status ? String(sharedLiquidSensors.getTempCByIndex(1), 1) : "--";
