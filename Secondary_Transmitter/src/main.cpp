 ...
 * 6-Pin Sensor: VCC->3.3V, GND->GND, SDA->21, SCL->22, CSB->3.3V, SDO->3.3V (Address 0x77)
 * ADS1115: VCC->3.3V, GND->GND, SDA->21, SCL->22
 * PH4502C: Po -> ADS1115 A0
 * DS18B20 (Liquid Temp): Data -> Pin 4 (Use 4.7k pullup to 3.3V), VCC->3.3V, GND->GND
 * UART to Main: TX(17) -> Main RX(16), RX(16) -> Main TX(17), GND -> GND
 */
...
class MyAdvertisedDeviceCallbacks: public NimBLEAdvertisedDeviceCallbacks {
    void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
        if (advertisedDevice->haveManufacturerData()) {
            ...
            if (payloadLength >= 23 && payload[0] == 0x52 && payload[1] == 0x41 && payload[2] == 0x50 && payload[3] == 0x54) {
                ...
                // --- 1. BATTERY ---
                // Based on your raw data, 100% (0x64) is at byte index 23
                if (payloadLength > 23) {
                    txData.pillBattery = payload[23];
                } else {
                    txData.pillBattery = payload[10]; // Fallback to v1 offset
                }

                // --- 2. TEMPERATURE ---
                uint16_t tempRaw = (payload[11] << 8) | payload[12];
                txData.pillTemp = (tempRaw / 128.0) - 273.15;
                
                // --- 3. SPECIFIC GRAVITY ---
                // Based on your raw data, 1.296 (0x44A2003A) starts at byte index 13
                uint32_t gravInt = (payload[13] << 24) | (payload[14] << 16) | (payload[15] << 8) | payload[16];
                float sgFloat;
                memcpy(&sgFloat, &gravInt, sizeof(float));
                
                // For this firmware version, SG is sent as density (1296.0)
                if (sgFloat > 200.0) {
                    txData.pillGravity = sgFloat / 1000.0;
                } else {
                    txData.pillGravity = sgFloat; 
                }

                // --- 4. RSSI (RADIO METADATA) ---
                txData.pillRSSI = advertisedDevice->getRSSI();

                Serial.print("Decoded -> Bat: "); Serial.print(txData.pillBattery);
                Serial.print("%, Temp: "); Serial.print(txData.pillTemp);
                Serial.print("C, Grav: "); Serial.println(txData.pillGravity, 4);
            }
        }
    }
};
...
void loop() {
    ...
    if (txData.sensor2Status == 0) {
        if (bme.begin(0x77)) txData.sensor2Status = 1;
        else if (bmp.begin(0x77)) txData.sensor2Status = 2;
    }
    
    if (txData.ds18Status == 0) {
        sensors.begin();
        if (sensors.getDeviceCount() > 0) txData.ds18Status = 1;
    }

    if (txData.adsStatus == 0) {
        if (ads.begin()) txData.adsStatus = 1;
    }
    ...
    // 5. UART Transmission (Persistence: txData holds last known values, including RAPT)
    txData.signature = 0xDEADBEEF;
    txData.checksum = calculateChecksum(txData);
    Serial2.write((uint8_t *)&txData, sizeof(txData));
