...
*   **Liquid Transfer Drain-to-Empty Protocol**:
    ...
    *   **Hardware Glitch and Noise Filtering**: Interrupt routines reject pulses with intervals under 14ms (rate > 9.5 L/min) and verify pin state is HIGH, eliminating motor EMI and dry-impeller air spinning on input-only GPIO 34.
    *   **Drain-to-Empty Confirmation**: The system monitors volume change over a rolling window. During bulk pumping, 20 seconds of volume delta < 0.15L indicates a stall (`TRANSFER_DRAIN_TIMEOUT_MS = 20000UL`). Once 90% of target liquid is transferred, a 10-second window (`TRANSFER_DRAIN_SETTLE_MS = 10000UL`) confirms complete chamber evacuation.
    *   **Anti-Runaway Volume Ceiling**: A transfer automatically terminates once transferred volume reaches the maximum plausible liquid limit (`max(target * 1.25 + 0.5L, target + 1.0L)`), preventing dry pump runaway and clamping final chamber volume.
    *   **Load Cell Initialization Volume Cap**: Any values of transferred liquid volume (`transferVolumeTransferred`, `transfer1Volume`, `transfer2Volume`, and destination `chamberVolume`) are strictly capped at the initial load cell weight (`transferStartWeight`) measured after brew initialization, ensuring flow sensor pulse drift never over-reports batch volume.
